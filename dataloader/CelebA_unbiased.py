...
class CelebA_unbiased(data.Dataset):
    def __init__(self, data_path, attr_path, syn_data_path, image_size, mode, selected_attrs, attrs_d):
        ...
        # print(type(self.origin_labels))
        # print(type(self.origin_labels_d))
        # print(self.origin_labels)
        # print(self.origin_labels.shape)
        # print(self.origin_labels_d.shape)
    ...
    def __getitem__(self, index):
        ...
        if index < self.origin_images.shape[0]:
            ...
            # img, att = Image.fromarray(self.syn_images[key][()]), self.syn_target_dict[key][0][np.newaxis]
            img = self.tf(Image.fromarray(syn_images[key][()]))
            # print(self.syn_target_dict)
