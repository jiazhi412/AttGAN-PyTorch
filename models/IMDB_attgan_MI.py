...
from models.IMDB_attgan import Model as A
...
class Model(A):
    def __init__(self, args):
        ...
        # MI
        self.mine = M(input_dim=(args.enc_dim * 2 ** (args.enc_layers-1) + self.dim_per_attr) * 7 * 7)
        self.mine.train()
    ...
    def pretrainMI(self, mine_loader, args):
        ...
        pretrain_it = 0
        print('Pretrain MINE')
        for img_a, sex_a, age_a in progressbar(mine_loader):
            print(age_a)
            print(age_a.size())
            print('dasjklsdasjskld')
            # prepare data
            img_a, age_a, age_a_, age_b, age_b_ = self.prepare_data(img_a, age_a, args)
            ...
            mine_loss = mi_criterion(zs_a[-1].detach().view(zs_a[-1].size(0), -1), a_tile.view(a_tile.size(0), -1), self.mine)
            mine_loss.backward()
